...
class CLDNN_mel(nn.Module):
    def __init__(self, mels=84, foresee: int=10, lookback: int=10):
        ...
        # DNN
        self.fc1 = nn.Linear(71*64*2+27*128*2, 512)
        self.fc2 = nn.Linear(512, 32)
        self.fc3 = nn. Linear(32, 2)
    ...
    def forward(self, inputs):
        time_inputs = self.bn1(inputs)
        # freq_inputs = inputs
        time_inputs = time_inputs.reshape(-1, 71, 320)


        # extract freq feature and 
        # batchsize = inputs.shape[0]
        # print(batchsize)
        mel_spec = transforms.MelSpectrogram(sample_rate=16000, n_fft=400, win_length=400, hop_length=284, n_mels=64).to(self.device)
...
class raw_data(Dataset):
    ...
    def frag_load(self, wav_path):
        ...
        for wav_file in wav_path:
            ...
            frags = torch.zeros((size, 1, 320*71))
            ...
            for i in range(size):
                ...
                index = num[i]
                frags[i, 0] = waveform[index*160:index*160+320*71]

            df = pd.read_csv(self.label_file)
            df = df['label_index'][df['id'] == wav_name]
            index = df.values[71:71+89800]
...
if __name__ == '__main__':
    ...
    val_path = '../data/val'
    model_cldnn = '../saved_models/cldnn_mel_longer_140.pkl'
    files = os.listdir(train_path)
    batch_size = 128


    wav_val = os.listdir(val_path)[:8] #['J1jDc2rTJlg.wav','Kb1fduj-jdY.wav','t1LXrJOvPDg.wav', 'yMtGmGa8KZ0.wav']
