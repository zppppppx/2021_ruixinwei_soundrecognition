...
class CLDNN_mel(nn.Module):
    def __init__(self):
        ...
        # DNN
        self.fc1 = nn.Linear(51*64*2+27*128*2, 512)
        self.fc2 = nn.Linear(512, 32)
        self.fc3 = nn. Linear(32, 2)
    ...
    def forward(self, inputs):
        time_inputs = self.bn1(inputs)
        # freq_inputs = inputs
        time_inputs = time_inputs.reshape(-1, 51, 320)


        # extract freq feature and 
        # batchsize = inputs.shape[0]
        # print(batchsize)
        mel_spec = transforms.MelSpectrogram(sample_rate=16000, n_fft=400, win_length=320, hop_length=204, n_mels=64).to(self.device)
...
class raw_data(Dataset):
    ...
    def frag_load(self, wav_path):
        ...
        for wav_file in wav_path:
            ...
            frags = torch.zeros((size, 1, 320*51))
            ...
            for i in range(size):
                ...
                index = num[i]
                frags[i, 0] = waveform[index*160:index*160+320*51]

            df = pd.read_csv(self.label_file)
            df = df['label_index'][df['id'] == wav_name]
            index = df.values[51:51+89800]
...
if __name__ == '__main__':
    ...
    val_path = '../data/val'
    model_cldnn = '../saved_models/cldnn_mel_longer_100.pkl'
    ...
    for j in range(200):
        wav_path = random.sample(files, 15) # randomly pick some files as the input
